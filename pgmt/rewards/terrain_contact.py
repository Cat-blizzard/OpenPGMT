...
论文的参考接触标签来自 **offline terrain-mesh queries**（把参考动作放到地形网格上
查询落点是否触地）；本仓库 npz 里的 `contacts` 来自 LAFAN1 足部位置的**速度阈值**
（`data/retarget_lafan1.py` 的 `contact_labels`）。两者**不同源**，而前者才是论文
口径 —— 因此 `reference_contact_match` 的数值有系统性偏差，报告里必须说明。
`semantics.py` 中该项的 note 记录了这一点。
